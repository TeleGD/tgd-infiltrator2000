...
public class Guard extends Character {
	...
	public void updateVisualField(){
		...
		for(int i =0;i<40;i++){
			...
			while(!(stop || games.guardSlalom.util.Collisions.distance(newPoint, this.getCenter())>300)){
				newPoint=games.guardSlalom.util.Collisions.prolongementDroite(this.getCenterX(), this.getCenterY(), a, b, av);
				av+=3;
				for (Wall w : games.guardSlalom.World.getWalls()){
	...
	public int summitToID(float x, float y){
		...
		float gWidth = Math.floorDiv(World.getWidth(), 32);
	...
	public float d(int ID,float xf, float yf){
		float x0,y0,gWidth;
		gWidth = Math.floorDiv(World.getWidth(), 32);
		x0 = ID%gWidth;
	...
	public boolean aStar(float x0, float y0, float xf, float yf){
		...
		int current, dest;
		final float gWidth = Math.floorDiv(World.getWidth(), 32);
	...
	private boolean wallAtCoords(int succ, ArrayList<Wall> walls) {
		int j = succ % Math.floorDiv(World.getWidth(), 32);
		int i = Math.floorDiv(succ, Math.floorDiv(World.getWidth(), 32));
